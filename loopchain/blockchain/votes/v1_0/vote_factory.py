...
from loopchain.blockchain.invoke_result import InvokeResultPool, InvokeResult
...
class BlockVoteFactory(VoteFactory):
    def __init__(self, invoke_result_pool: InvokeResultPool, signer):
        self._invoke_result_pool: InvokeResultPool = invoke_result_pool
    ...
    async def create_vote(self, data_id: bytes, commit_id: bytes, epoch_num: int, round_num: int) -> BlockVote:
        ...
        invoke_result: InvokeResult = self._invoke_result_pool.get_invoke_result(data_id)
        ...
        signature = self._get_signature(
            ...
            data_id=data_id,
            epoch_num=epoch_num,
            state_hash=invoke_result.state_hash,
            receipt_hash=invoke_result.receipt_hash,
            round_num=round_num,
            timestamp=timestamp
        )
